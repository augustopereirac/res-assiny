...
(function () {
  ...
  function telaSetup() {
    ...
    render(`
      ...
      <div class="modo-toggle"><span class="on">📱 Um celular</span><a href="top100-sala.html?estilo=${cfg.estilo}&n=${cfg.tamanho}">📲 Vários celulares</a></div>
  ...
  function listaChutes(revelarTudo) {
    ...
    return `<div class="card"><span class="label">Já falados</span><div class="said">
      ${jogo.chutes.slice().reverse().map(c => {
        ...
        if (oculto) return `<span class="said-item">${esc(c.texto)}<span class="by">${esc(c.jogador)}</span></span>`;
        return `<span class="said-item ${c.item ? 'ok' : 'miss'}">${c.item ? '#' + c.item.pos + ' ' + esc(c.item.nome) : esc(c.texto || 'passou')}<span class="by">${esc(c.jogador)}</span></span>`;
      }).join('')}
    </div></div>`;
  }
