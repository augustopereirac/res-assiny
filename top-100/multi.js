...
(function () {
  ...
  Sala.telaEntrada(app, 'Top 100', './', (nome, codigo, criar) => {
  ...
  function jaFalados() {
    ...
    return `<div class="card"><span class="label">Já falados</span><div class="said">${estado.chutes.slice().reverse().map(c => c.status === 'passou' && !c.item && estado.fase !== 'final'
      ? `<span class="said-item">${esc(c.texto)}</span>`
      : `<span class="said-item ${c.item ? 'ok' : 'miss'}">${c.item ? '#' + c.item.pos + ' ' + esc(c.item.nome) : esc(c.texto || 'passou')}</span>`).join('')}</div></div>`;
  }
